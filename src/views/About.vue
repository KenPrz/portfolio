        ...
        <div class="flex flex-col md:flex-row gap-10 items-center">
          ...
          <div class="md:w-1/3 flex-shrink-0">
            <div class="relative rounded-2xl overflow-hidden aspect-square group">
              <img src="/me.jpg" alt="Ken Perez" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
          ...
          <div class="md:w-2/3 space-y-4">
            ...
            <p class="text-dark-300 leading-relaxed">
              I am a Full-Stack Developer with over 3 years of experience specializing in PHP, JavaScript, and MySQL. My journey in tech spans from engineering IoT solutions to architecting complex, database-driven web platforms.
            </p>
            <p class="text-dark-300 leading-relaxed">
Currently, I am focused on mastering software architecture, system design, and maintainable design patterns to build scalable, high-performance digital experiences.
            </p>
            <!-- Social Links -->
    ...
    <section id="skills" class="reveal mb-24" :class="{ 'active': isInView['skills'] }">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div v-for="(skillGroup, index) in skills" :key="index" 
          ...
          <div class="space-y-4">
            <div v-for="skill in skillGroup.items" :key="skill.name">
              ...
              <div class="skill-bar-bg">
                <div class="skill-bar-fill" :style="{ width: isInView['skills'] ? `${Math.min(skill.years * 25, 100)}%` : '0%' }"></div>
              </div>
            </div>
          </div>
        </div>
            ...
            <div class="flex flex-col sm:flex-row sm:items-start gap-2 mb-3">
              <div>
                ...
                <p class="text-accent text-sm font-mono">
                  <a v-if="exp.companyLink" :href="exp.companyLink" target="_blank" class="hover:underline">{{ exp.company }}</a>
                  <span v-else>{{ exp.company }}</span>
                </p>
              </div>
              <span class="text-xs font-mono text-dark-400 sm:ml-auto whitespace-nowrap bg-dark-800 px-3 py-1 rounded-full">
                {{ exp.period }}
              </span>
            </div>
          ...
          <div v-for="(edu, index) in education" :key="index" class="mb-6 last:mb-0 border-l-2 border-accent/30 pl-4">
            ...
            <ul v-if="edu.achievements?.length" class="space-y-1 mt-2">
              <li v-for="(a, aIdx) in edu.achievements" :key="aIdx" class="flex items-start gap-2">
                <svg class="w-3.5 h-3.5 text-accent flex-shrink-0 mt-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
                  <polyline points="20 6 9 17 4 12" />
                </svg>
          ...
          <div class="space-y-4 mb-6">
            <a v-for="(cert, index) in certifications" :key="index" :href="cert.link" target="_blank" 
              ...
              <div>
                ...
                <p class="text-dark-400 text-xs mt-0.5 flex items-center gap-1 group-hover:text-accent transition-colors">
                  View certificate 
                  <svg class="w-3 h-3 group-hover:translate-x-1 transition-transform" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <section id="cta" class="reveal mb-10" :class="{ 'active': isInView['cta'] }">
      <div class="glass-card p-10 text-center relative overflow-hidden">
        ...
        <div class="relative z-10">
          ...
          <p class="text-dark-300 mb-8 max-w-lg mx-auto">I'm always open to discussing new projects, creative ideas, or opportunities to be part of your vision.</p>
